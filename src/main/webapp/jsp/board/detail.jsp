...
<body>
	...
	<div class="container">
		...
		<div class="input-group">
	    	...
	    	<button type="submit" class="input-group-text" id="btnGroupAddon">✔</button>
	  	</div>
		</form>
		
		<!-- 답변 리스트 -->
		<c:forEach items="${ commentList }" var="comment" varStatus="loop">
			<div style="margin-left:<c:out value="${30*comment.depth}"/>px">
				<c:out value="${ comment.writer }"/> 
				<c:out value="${ comment.writingDate }"/> 
				<br>
				<c:out value="${ comment.content }"/> 
				<span class="toggleBtn">답글달기</span>
